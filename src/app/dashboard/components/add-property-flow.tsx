...
function LocationSearch({ onPlaceSelect, onUseMyLocation, locating, displayAddress }: {
  ...
  const showGeoOption = showMyLocation && !showDropdown && !query.trim();
    ...
    <div className="relative">
      <div className="relative">
        ...
        <input type="text" value={query}
          ...
          onFocus={() => { if (!query.trim() && onUseMyLocation) setShowMyLocation(true); }}
      ...
      {/* Dropdown: search results OR "use my location" */}
      {(showDropdown || showGeoOption) && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-neutral-200 rounded-xl shadow-lg z-50 max-h-60 overflow-y-auto">
          {/* "Usa la mia posizione" option */}
          {showGeoOption && (
            <button onClick={handleMyLocation} disabled={locating}
              className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-neutral-50 transition-colors cursor-pointer border-b border-neutral-100 disabled:opacity-60">
              {locating ? (
          ...
          {/* Search predictions */}
          {showDropdown && predictions.map((p) => (
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        {GOOGLE_MAPS_API_KEY ? (
          <div className="relative">
            ...
            {/* Search with integrated "use my location" dropdown */}
            <div className="absolute top-4 left-4 right-4 z-10">
              <LocationSearch onPlaceSelect={handlePlaceSelect} onUseMyLocation={handleUseMyLocation} locating={locating} />
            </div>
            {/* Error feedback */}
            {geoError && (
